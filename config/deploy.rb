...
# Default branch is :master or :stable
...
append :linked_dirs, "log", "storage", "tmp/pids", "tmp/cache", "tmp/sockets", "tmp/storage", 
       "vendor/bundle", "public/assets", "public/uploads"
...
# By default, nginx_config will be executed with :web role.
# But you can assign it to a different role:
set :puma_nginx, :app
...
# default value: nil
set :delayed_job_queues, %w[default mailer mailers tracking]
  ...
  task :update_cron do
    on roles(:app) do
      within current_path do
        execute :bundle, :exec,
                "whenever --set environment=#{fetch(:rails_env)} --update-crontab #{fetch(:application)}"
      end
    end
  end
